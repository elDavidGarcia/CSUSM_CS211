...
int main()
{
  ...
  level = getNumbersInRange(1, 5, "Invalid level. Enter 1 to 5: "); //from inputCheck.h
  ...
  while(q.getSize() < 10 && !q.isEmpty() && numCorrect < 100)
    {
      ...
      cout << op1 << " " << opr << " " << op2 << " = ";
      answer = getNumbersInRange(0, 400, "Invalid answer. Enter your answer again: ");//from inputCheck.h
...
//do the math to get correct answer
int correctAnswer(int op1, char optr, int op2)
{
  switch(optr)
    {
...
expr makeQuestion()
{
  ...
  return e; //Return the newly made question
}
